...
def adapt_qaoa(n,
    ...
    for p in range(0, layer):
        ...
        x0 = {}
        x0[0] = (0.001)
        x0[2] = (-0.0001, -0.785, 2.01)
        x0[3] = (-0.0001, -0.785, 2.01, -2.50)
        min_options = {'gtol': theta_thresh, 'disp': False}

        trial_model = tUCCSD(hamiltonian, ansatz_mat, reference_ket, parameters)

        opt_result = scipy.optimize.minimize(trial_model.energy, x0[2*p], jac=trial_model.gradient,
                                             options=min_options, method='Nelder-Mead', callback=trial_model.callback)
        ...
        opt_result = scipy.optimize.minimize(trial_model.energy, x0[2*p+1], jac=trial_model.gradient,
